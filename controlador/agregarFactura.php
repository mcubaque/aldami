...
// insert query
$query = "INSERT INTO factura (id_producto, cantidad, id_usuario, creado) VALUES (?,?,?,?)";
...
if($ok){
    header('Location: ../vista/venta.php?action=added&prodid=' . $prodid . '&prodmarca=' . $prodmarca);
}
 
// if database insert failed
else{
     header('Location: ../vista/venta.php?action=failed&prodid=' . $prodid . '&prodmarca=' . $prodmarca);
}
