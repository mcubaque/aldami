...
// delete query
$query = "UPDATE cart_items SET cantidad=? WHERE id_producto=? AND id_usuario=?";
...
if($ok){
    ...
    header('Location: ../vista/carro.php?action=quantity_updated&prodid=' . $prodid . '&prodmarca=' . $prodmarca);
}
 
// if remove failed
else{
    // redirect and tell the user it failed
    header('Location: ../vista/carro.php?action=failed&prodid=' . $prodid . '&prodmarca=' . $prodmarca);
}
?>